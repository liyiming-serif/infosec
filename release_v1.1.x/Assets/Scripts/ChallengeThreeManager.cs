...
public class ChallengeThreeManager: MonoBehaviour { 
	...
	void Start () {
		...
		playerInbox = new Inbox (dropdownBoxLoc, boxPositions);
		...
		ground = new MemoryBar (memoryPickupPoses, boxPositions);
		ground.acceptBoxAt (0, new Box(yellowHash, dataHash));
	...
	void Update () {
		...
		if (playerOldCounter != player.counter) { //Player movement is done
			...
			case 3:
				...
				if (!ground.hasBoxAt(index)) {
					this.FailRunning ("No box in this cell.", bossFeedback);
				} else {
					player.PickupBox (ground.cloneBoxAt(index, MemoryBar.ReadOwner.Boss));
					playerCmd++;
					RunCommand ();
				}
				break;
