...
public class NewChallengeSixManager: MonoBehaviour { 
	...
	void RunCommand()
	{
		...
		if (playerCmd == commands.Count) {
			playerCmd--;
			this.FailRunning ("I haven't got the blue boxes in reverse order!", bossFeedback);
			return;
		}
	...
	void FailRunning(string message, GameObject feedback)
	{
		...
		if (feedback.Equals (spyFeedback)) {
			playerCmd--;
		}
	...
	void Update () {
		...
		if (spyOldCounter != spy.counter) { //Spy movement is done.
			if (spyOldCounter + 1 == spy.counter) {
				...
				this.FailRunning ("Aha! Your boss has a private box'" + spyOutbox.boxesRef[spyOutbox.GetCount()-1].data.ToString() + "'!", spyFeedback);
				...
				if (!player.IsCarryingBox()) {
					...
					if (commands [playerCmd].actTo.GetCaptionText () == bossName) {
						...
						if (receivedData == dataE) {
							if (bossOutbox.boxesRef[0].data == dataY) {
