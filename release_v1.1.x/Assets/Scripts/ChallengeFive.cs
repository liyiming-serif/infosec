...
public class ChallengeFive : HackingChallengeTemplate {
	...
	[SerializeField] protected AnimatorController distrustPlayer;
	...
	protected override void Reset ()
	{
		...
		distrustPlayer.ResetAnimator ();
		distrustCMDNo = -1;
		distrustPlayerState = RunningState.Inactive;
		distrustOldCounter = distrustPlayer.counter;
	}
	...
	protected override bool RunPlayerCommand(){
		if (!CheckPlayerReady ()) {
			return false;
		}
		...
		if(distrustPlayerState != RunningState.Inactive){
			...
			if (topCodeToRun == TopCommand.Code.Inbox) {
				distrustPlayer.SetEndPosition (new Vector2 (-436f, -111f));
			} else if (distrustSubCode [distrustCMDNo] != SubCommand.Code.NoAction) {
				SetEndPositionBySubCMD (distrustPlayer, distrustSubCode [distrustCMDNo]);
			}
		}
	...
	void Update () {
		...
		if ((distrustPlayerState != RunningState.Inactive) && (distrustOldCounter != distrustPlayer.counter)) {
			distrustOldCounter = distrustPlayer.counter;
			...
			case TopCommand.Code.Inbox:
				...
				d.dataStr = "?";
				distrustPlayer.PickupData (d);
				break;
			case TopCommand.Code.Outbox:
				d = distrustPlayer.SendData ();
				...
				break;
			case TopCommand.Code.Load:
				distrustPlayer.PickupData (memoryBar.CloneDataAt ((int)distrustSubCode [distrustCMDNo]));
				break;
			case TopCommand.Code.Store:
				memoryBar.AcceptDataAt ((int)distrustSubCode [distrustCMDNo], distrustPlayer.CloneData());
				break;
			}
			distrustCMDNo += 1;
			distrustPlayerState = RunningState.Ready;
