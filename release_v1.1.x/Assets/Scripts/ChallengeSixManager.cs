...
public class ChallengeSixManager: MonoBehaviour { 
	...
	void RunCommand()
	{
		...
		if (playerCmd == commands.Count) {
			playerCmd--;
			this.FailRunning ("I haven't got every two boxes in reverse order!", bossFeedback);
			return;
		}
			...
			case 2:
				if (!player.IsCarryingBox ()) {
					...
					if (commands [playerCmd].actTo.GetCaptionText () == bossName) {
						...
						case dataE:
							if (outboxState == 0) {
								FailRunning ("I expect the first box to be 'Y'.", bossFeedback);
							} else if (outboxState == 1) {
								outboxState++;
								playerCmd++;
								Invoke ("RunCommand", 0.6f);
							} else if (outboxState == 2) {
								FailRunning ("I expect the third box to be '$'.", bossFeedback);
							} else if (outboxState == 3) {
								FailRunning ("I expect the forth box to be '#'.", bossFeedback);
							}
							break;
						case dataY:
							if (outboxState == 0) {
								...
								FailRunning ("I expect the second box to be 'E'.", bossFeedback);
							} else if (outboxState == 2) {
								FailRunning ("I expect the third box to be '$'.", bossFeedback);
							} else if (outboxState == 3) {
								FailRunning ("I expect the forth box to be '#'.", bossFeedback);
						...
						case dataHash:
							if (outboxState == 0) {
								...
								succeedRunning ("Thanks for your hard working!.", bossFeedback);
							}
							break;
						case dataDollar:
							if (outboxState == 0) {
						...
						if (receivedData == dataE || receivedData == dataY) {
							this.FailRunning ("Aha! Your boss has a private box'" + receivedData.ToString () + "'!", spyFeedback);
						}else if (receivedData == dataBossDollar) {
							FailRunning ("Aha! Your boss is processing box'$'!", spyFeedback);
						} else if(receivedData == dataBossHash) {
							FailRunning ("Aha! Your boss is processing box'#'!", spyFeedback);
